...
def add_gateway(request):
    ...
    assert CustomSubDir.objects.filter(dir_code_id=code).count() == 0 and \
           GatewayInfo.objects.filter(code=code).count() == 0, FieldError("code", '国标编码不能和现有网关/目录重复。')
    assert dir_code_id == '0' * 20 or CustomSubDir.objects.filter(dir_code_id=dir_code_id).count() == 1, FieldError("dir_code_id",
                                                                                                          "父目录不存在！请重试！")
    if dir_code_id == '0'* 20:
        dir_code_id = ''
...
def del_gateway(request):
    ...
    post_info = request.POST
    code = post_info.get("code")
    DeviceNodeInfo.objects.filter(parent_node_id=code).delete()   # 清楚设备信息
